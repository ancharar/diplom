"""Тесты VK-токенов и публикации (unittest)."""
...
class VKTokenTest(TestCase):
    ...
    def test_save_token(self):
        ...
        self.assertTrue(
            VKToken.objects.filter(user=self.user).exists(),
        )

    def test_delete_token(self):
        """Удаление VK-токена возвращает 204."""
        VKToken.objects.create(
            user=self.user, access_token='test',
        )
    ...
    def test_delete_nonexistent_token(self):
        """Удаление несуществующего токена → 400."""
        response = self.client.delete('/api/v1/vk/token/')
        self.assertEqual(
            response.status_code,
...
class VKPublishTest(TestCase):
    ...
    def setUp(self):
        ...
        VKToken.objects.create(
            user=self.user, access_token='mock_token',
        )
        self.client.force_authenticate(user=self.user)
    ...
    def test_publish_success(self, mock_post: MagicMock):
        ...
        self.assertEqual(
            response.data['vk_post_id'], 12345,
        )
